      ...
      try {
        ...
        if (!result.success) {
          toast({
            ...
            description:
              result.error ||
              "No se pudo completar la asociación con tu profesional."
          });
        }
      } catch (error) {
        ...
        toast({
          variant: "destructive",
          title: "Error Crítico de Invitación",
          description: "No se pudo procesar tu invitación."
        });
      }
    }

    const destination =
      role === "profesional" ? "/pro-dashboard" : "/dashboard";
